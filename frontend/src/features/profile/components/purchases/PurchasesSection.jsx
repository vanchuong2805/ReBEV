      ...
      <CardContent>
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="all" className="space-y-4">
            ...
            {all.map(o => {
              const s = getStatus(o)
              if (s === 'Chờ xác nhận')   return <PendingPurchaseCard    key={o.id} purchase={o} />
              if (s === 'Đang xử lý')     return <ProcessingPurchaseCard key={o.id} purchase={o} />
              if (s === 'Đang vận chuyển')return <ShippingPurchaseCard   key={o.id} purchase={o} />
              if (s === 'Hoàn tất')       return <SuccessPurchases       key={o.id} purchase={o} />
              if (s === 'Đã huỷ')         return <CanceledPurchaseCard   key={o.id} purchase={o} />
              return null
            })}
          </TabsContent>
