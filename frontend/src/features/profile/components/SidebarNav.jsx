...
import { 
  BarChart3, Car, FileText, Heart, ShoppingCart, Package, Edit3, LogOut, Wallet 
} from 'lucide-react'
  ...
  const Item = ({ path, icon: Icon, label }) => (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-gray-50 transition-colors ${
        current === path ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-700' : 'text-gray-700'
      }`}
    >
    ...
    <Card className='sticky top-4'>
      ...
      <CardContent className='p-0'>
        <nav className='space-y-1'>
          ...
          <Item path='dashboard' icon={BarChart3} label='Thống kê' />
          <Item path='logout' icon={LogOut} label='Đăng xuất' />
        </nav>
      </CardContent>
    </Card>
  )
