...
const CanceledSaleCard = ({ sale = {}, onDetail, onRelist, onSupport }) => {
  const { title, category_name, thumbnail_url, create_at, status_vi='Đã huỷ', order_type_vi, total_amount, status_note } = sale
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between mb-3">
            <div>
              ...
              <p className="text-sm text-red-700 mt-2">{status_note ? `Lý do huỷ: ${status_note}` : 'Đơn đã huỷ.'}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="grid grid-cols-3 gap-2 w-full sm:w-[420px]">
              ...
              <Button className="h-10 w-full" variant="outline" onClick={() => onRelist?.(sale)}>Đăng lại</Button>
              <Button className="h-10 w-full" variant="outline" onClick={() => onSupport?.(sale)}>Hỗ trợ</Button>
            </div>
          </div>
        </div>
      </div>
...
export default CanceledSaleCard
